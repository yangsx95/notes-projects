...
public class ConsumerDemo {
    ...
    public void before() {
        ...
        prop.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, IntegerDeserializer.class.getName()); // value 反序列化
        prop.put(ConsumerConfig.GROUP_ID_CONFIG, "g1-interceptor"); // 消费者组
        prop.put(ConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, PrintInfoConsumerInterceptor.class.getName()); // 指定消费拦截器
        consumer = new KafkaConsumer<>(prop);
        consumer.subscribe(Collections.singleton("topic-interceptor"));
    }
    ...
    public void receive() {
        while (true) {
            ConsumerRecords<String, Integer> records = consumer.poll(Duration.ofSeconds(1));// 每隔一秒， 取出一条消息
